...
namespace HoloToolkit.Examples.InteractiveElements
{
    ...
    /// </summary>
    public class SliderGestureControlNew : GestureInteractiveControl
    {
        ...
        protected override void Awake()
        {
            ...
            mStartSliderPosition = mStartCenter + Vector3.right * mSliderMagnitude / 2;
            ...
            if (SliderFill != null)
            {
                ...
                mSliderWidth = SliderFill.transform.InverseTransformVector(SliderFill.GetComponent<Renderer>().bounds.size).y;
            }
            ...
            mSliderVector = SliderBar.transform.InverseTransformPoint(mStartCenter + SliderBar.transform.forward * mSliderMagnitude / 2) - SliderBar.transform.InverseTransformPoint(mStartCenter - SliderBar.transform.forward * mSliderMagnitude / 2);
            AlignmentVector = SliderBar.transform.forward;
        ...
        public void Start()
        {
            ...
            //mainSlider.onValueChanged.AddListener(ValueChange);
        }
        ...
        */
        ...
        {
            if (AlignmentVector != SliderBar.transform.forward)
            {
                mSliderVector = SliderBar.transform.InverseTransformPoint(mStartCenter + SliderBar.transform.forward * mSliderMagnitude / 2) - SliderBar.transform.InverseTransformPoint(mStartCenter - SliderBar.transform.forward * mSliderMagnitude / 2);
                AlignmentVector = SliderBar.transform.forward;
        ...
        private void UpdateVisuals()
        {
            ...
            Debug.Log(mStartSliderPosition + " " + mSliderMagnitude + " " + mDeltaValue);
            mKnobVector = mStartSliderPosition + Vector3.left * mSliderMagnitude * mDeltaValue;
            ...
            if (SliderFill != null)
            {
                ...
                scale.y = mSliderFillScale.y * mDeltaValue;

                Vector3 position = Vector3.forward * (mSliderWidth * 0.5f - mSliderWidth * mDeltaValue * 0.5f); // left justified;

                if (Centered)
                {
                    if (SliderValue < 0)
                    {
                        position = Vector3.forward * (mSliderWidth * 0.5f - mSliderWidth * 0.5f * (mDeltaValue + 0.5f)); // pinned to center, going left
                        scale.y = mSliderFillScale.y * (1 - mDeltaValue / 0.5f) * 0.5f;
                    }
                    else
                    {
                        position = Vector3.forward * ((mSliderWidth * 0.5f * (mDeltaValue - 0.5f))); // pinned to center, going up
                        scale.y = mSliderFillScale.y * ((mDeltaValue - 0.5f) / 0.5f) * 0.5f;
                    }
                }
            ...
            if (Label != null)
            {
                ...
                {
                    Label.text = displayValue.ToString("F2");
                }
            }
        }
